...
static struct Command commands[] = {
	...
	{ "continue", "Continue the current environment", mon_continue },
	{ "stepi", "Step forward one instruction in the user environment", mon_stepi },
};
...
{
	if(tf == NULL){
		cprintf("continue: No runnable environment\n");
		return 0;
	}
	tf->tf_eflags &= ~FL_TF;		// set TF to 0
	return -1;
}
...
{
	if(tf == NULL){
		cprintf("stepi: No runnable environment\n");
		return 0;
	}
	tf->tf_eflags |= FL_TF;		// set TF to 1
	return -1;
}
