...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 4: Your code here.
	//cprintf("debug#page_fault_handler start, va = 0x%08x\n", fault_va);
	if(curenv->env_pgfault_upcall != NULL){
		uintptr_t ux_esp = 0;
		if(tf->tf_esp < UXSTACKTOP && tf->tf_esp >= UXSTACKTOP - PGSIZE)
			ux_esp = tf->tf_esp - sizeof(struct UTrapframe) - 4;
		else ux_esp = UXSTACKTOP - sizeof(struct UTrapframe);
		// 检查对ux_esp开始的栈帧空间，该用户是否有权限访问
		user_mem_assert(curenv, (void *)ux_esp, sizeof(struct UTrapframe), PTE_W);
		lcr3(PADDR(curenv->env_pgdir));		// 切换成用户的页目录
		/*	空白是用于填充eip的，无需清0
		if(ux_esp != UXSTACKTOP - 4){
			*((int *)(ux_esp + sizeof(struct UTrapframe))) = 0;
		}	*/
		struct UTrapframe utrapf;
		utrapf.utf_fault_va = fault_va;
		utrapf.utf_err = tf->tf_err;
		utrapf.utf_regs = tf->tf_regs;
		utrapf.utf_eip = tf->tf_eip;
		utrapf.utf_eflags = tf->tf_eflags;
		utrapf.utf_esp = tf->tf_esp;
		*((struct UTrapframe *)ux_esp) = utrapf;
		lcr3(PADDR(kern_pgdir));		// 切回内核页目录
		tf->tf_esp = ux_esp;
