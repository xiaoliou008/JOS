...
alloc_block(void)
{
	...
	for(uint32_t i = 0; i * 32 < super->s_nblocks; i++){
		if(bitmap[i] != 0){
			uint32_t cnt = 0;
...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	if(filebno >= NDIRECT + NINDIRECT){
		cprintf("file_block_walk: out of range\n");
		return -E_INVAL;
	}
	if(filebno < NDIRECT){
		if(ppdiskbno != NULL)
...
//		cprintf("file_block_walk: not found\n");
		return -E_NOT_FOUND;
	}
	else if(f->f_indirect == 0){
		f->f_indirect = alloc_block();
		if(f->f_indirect < 0){
			f->f_indirect = 0;		// recover
			cprintf("file_block_walk: no disk\n");
			return -E_INVAL;
		}
		// clear the block
		memset((char *)diskaddr(f->f_indirect), 0, BLKSIZE);
	}
	if(ppdiskbno != NULL)
...
//			*blk = (char *)(DISKMAP + (*ppdiskbno) * BLKSIZE);
	...
	*ppdiskbno = alloc_block();
	if(*ppdiskbno < 0) return *ppdiskbno;
	if(blk != NULL)
		*blk = (char *)diskaddr(*ppdiskbno);
	return 0;
